...
def main():
    ...
    print("\n【1. 查找黄浦区数据】")
    cenxi = loader.get_region_by_name('黄浦')
    if cenxi:
    ...
    test_points = [
        ...
        (111.0, 22.95, "黄浦区东部"),
        (110.6, 23.0, "黄浦区北部"),
        (110.9508, 22.9191, "黄浦区区附近（百度坐标）"),
    ]
    
    for lon, lat, desc in test_points:
        result = query.query(lon, lat)
        status = "✓" if result['district'] and '黄浦' in result['district'] else "✗"
    ...
    for lon, lat, desc in outside_points:
        result = query.query(lon, lat)
        is_cenxi = result['district'] and '黄浦' in result['district']
